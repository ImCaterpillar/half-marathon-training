...
export const POST = withApiAuth(async (request: NextRequest) => {
  ...
  const preview = parseStructuredWorkoutExport(payload);
  ...
  for (const row of preview.rows) {
    ...
    if (error) throw new Error(`Failed to preview structured workout import: ${error.message}`);
  ...
  return ok({
    ...
    preview: {
      ...
      rollback: "A backup entry will be written to plan_versions before the structured import is applied.",
    },
    conflicts,
  });
});
